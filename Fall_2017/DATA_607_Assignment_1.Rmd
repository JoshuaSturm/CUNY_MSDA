...
#### Next, we want to rename the values in the table
#### Note that I was having issues here. One solution I found online was to use the data.table library

#{r eval=TRUE}
#mm <- as.matrix(mDB2)
# mm[mm == "e"] <- "Edible"
# #mDB2$Class[mDB2$Class == "e"] <- "Edible"
#mDB2 <- as.data.frame(mm)
# ```

```{r}
#library(data.table)
#DT1 <- as.data.table(mDB2, function(e){replace(e, e == "e", "Edible")})
#mDB2[] <- lapply(mDB2, as.character)
#mDB2$Class[([mDB2$Class == "e"] <- "Edible"),([mDB2$Class == "p"] <- "Poisonous")]
#recode(mDB2$Class, e = "Edible", p = "Poisonous")
